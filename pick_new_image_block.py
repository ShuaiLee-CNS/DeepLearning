...
# log_path1 = "/home/DL/LP/result/lllog/log.txt"
# logfile1 = open(log_path1, 'w+')
...
def determine_the_block(allfile_wgs_lat_lon, allfile_jpg, allfile_txt, point_lon_lat, block_length, width, new_file_jpg_dir, new_file_txt_dir, logfile):
    ...
    if condition2 == 0:
        ...
        print >> logfile, "In function determine_the_block \n\tcondition2 = %s,\tlen(condition1) = %s" % (condition2, len(condition1))
        d_0 = allfile_jpg[condition2]
        d_1 = allfile_txt[condition2]
        e = allfile_wgs_lat_lon[condition2]
        e = mat(e)
        print >> logfile, "\t len(allfile_jpg) = %s,\tlen(allfile_txt) = %s,\tlen(allfile_wgs_lat_lon) = %s" % (len(allfile_jpg), len(allfile_txt), len(allfile_wgs_lat_lon))
        print >> logfile, "\te = %s" % str(e)
        ...
        # print("get_include_block ok!")
        panduan1 = False
    return new_file_jpg_dir, new_file_txt_dir, panduan1
...
def pick_the_block_from_new_image(new_jpg, new_file_txt_dir, point_lon_lat, block_length, width=256):
    ...
    region = (int(ceil(clumn - block_length1)), int(ceil(row - block_length1)), int(ceil(clumn + block_length1)), int(ceil(row + block_length1)))
...
def main_function(file_path, path_veg, saveToPath, block_length, number_block):
    ...
    # name = name
    # i = iii
    log_path = saveToPath + "/log/%s_%d_log.txt" % (hostname, pid)
    ...
    for i in range(1, number_block + 1):
        ...
        panduan = True
        while panduan:
            point_lon_lat2 = [random.uniform(98.15, 113.85), random.uniform(34.15, 40.85)]
            print >> logfile, "point_lon_lat2 = %s" % str(point_lon_lat2)

            new_jpg_dir, new_txt_dir, panduan1 = determine_the_block(allfile_wgs_lat_lon, allfile_jpg, allfile_txt, point_lon_lat2, block_length, 256, [], [], logfile)
            panduan = panduan1
        new_jpg = image_joint(new_jpg_dir)

        block_Img = pick_the_block_from_new_image(new_jpg, new_txt_dir, point_lon_lat2, block_length)
        label1 = get_point_veg_type(veg_type, point_lon_lat2)

        # block_Img, label1 = new_block_img_label(file_path, point_lon_lat2, block_length, veg_type)
        # point_lon_lat2 = point_lon_lat2.tolist()
        block_Img.save(saveToPath + "/img/%s_%d_%s.jpg" % (hostname, pid, name))
        # point_lon_lat1.append(point_lon_lat2)
        # label.append(label1)
        saveToPath_jpg.append(saveToPath + "/img/%s_%d_%s.jpg" % (hostname, pid, name))
        # print("Completed : %d" % i)
        point_lon_lat = str(point_lon_lat2)
        point_lon_lat = point_lon_lat.replace("[", "")
        point_lon_lat = point_lon_lat.replace("]", "")
        point_lon_lat = point_lon_lat.replace(", ", ",")
        lat_log_files = open(saveToPath + "/lat_log/%s_%d_lat_log.txt" % (hostname, pid), "a")
        if i == 1:
        ...
        labels = labels.replace("]", "")
            #labels = labels.replace(", ", "\n")
        f = open(saveToPath + "/labels/%s_%d_labels.txt" % (hostname, pid), "a")
        if i == 1:
            f.write(labels)
        else:
            f.write("\n%s" % labels)
        f.close()
        print>> logfile, "Completed : %d\n" % i
    # print("Congratulations!!")
    logfile.close()
    # logfile1.close()
    return saveToPath_jpg
